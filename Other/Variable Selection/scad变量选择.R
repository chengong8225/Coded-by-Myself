...
estimate.beta.by.cd <- function(x, y, beta, lambda, Gamma)
{
  ...
  for(i in 1:ncol(x))
  {
    ...
    if (abs(tmp)<= lambda*2)
      beta.new[i] <- sign(tmp)*max(0,abs(tmp)-lambda)
    else if (abs(tmp)>= lambda*Gamma)
      beta.new[i] <- tmp
    else
      beta.new[i] <- sign(tmp)*max(0,abs(tmp)-Gamma*lambda/(Gamma-1))/(1-1/(Gamma-1))
  }
  return(beta.new)
}

get.lambda.by.cv <- function(x, y, fold, iteration)  #x是第一列全为1的设计矩阵
{
  N <- nrow(x)
  p <- ncol(x)
  ## fold <- 10
...
library(ncvreg)
scad.object <- ncvreg(l$x, l$y, penalty = "SCAD", lambda = lambda.opt, gamma = 3)
scad.object$beta
